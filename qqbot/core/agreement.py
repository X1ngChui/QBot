...
the text file are mandatory config (settings.yaml `agreement:`); startup loads
text and version together. A deployment without an agreement fails at load—there
is no placeholder state. The in-memory set caches only "yes" answers—a "no" must
stay re-checkable the moment /agree lands.
"""
...
_AGREED: set[tuple[GroupId, str, int]] = set()
...
_PROMPTED: dict[tuple[GroupId, str], float] = {}
...
async def ok(group_id: GroupId, user_id: str) -> bool:
    """Whether this account has accepted the current agreement in this group."""
    v = version()
    key = (group_id, user_id, v)
    if key in _AGREED:
        return True
    if await repo.has_agreed(group_id, user_id, v):
        _AGREED.add(key)
        return True
    return False


async def accept(group_id: GroupId, user_id: str) -> bool:
    ...
    v = version()
    fresh = await repo.record_agreement(group_id, user_id, v)
    _AGREED.add((group_id, user_id, v))
    # An accepted member never sees the pointer again; their cooldown entry
    # is dead weight from here on.
    _PROMPTED.pop((group_id, user_id), None)
    return fresh


def should_prompt(group_id: GroupId, user_id: str) -> bool:
