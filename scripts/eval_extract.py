...
def main_checks(cands: list) -> list[tuple[str, bool, str]]:
    ...
    def payloads(kind=None):
        return [c.payload for c in cands if kind is None or c.payload.get("_tool") == kind]

    alias_ok = any(
        p.get("alias") == "老王" and p.get("account") == 1 for p in payloads("record_alias")
    )
    joke = [p for p in payloads() if "秦始皇" in str(p)]
    note = [p for p in payloads() if "王小锤" in str(p) or "周末上线" in str(p)]
    term = [p for p in payloads("record_group_term") if p.get("term") == "切片"]
    moved = any(
        p.get("account") == 1
        and p.get("predicate") == "lives_in"
        and "无锡" in str(p.get("object", ""))
        for p in payloads("record_fact")
    )
    codes_ok = all(
        isinstance(p.get("account"), int) and p["account"] in (1, 2, 3)
        for p in payloads()
        if "account" in p
    )
    ...
    styled = [s for s in episodes if re.search(r"又一轮|考验|属.{0,10}(测试|考察)", s)]
    self_named = [p for p in payloads("record_alias") if p.get("alias") in ("阿旺", "旺财")]
    own_derived = [p for p in payloads() if "占座" in str(p) or "柴犬" in str(p)]
    ...
    return [
        ("a reused known alias is re-recorded (confirmation evidence)", alias_ok, ""),
        (
        ...
        (
            ...
            any("微博" in o and "dachui2020" in o for o in handles),
            str(handles),
        ),
        (
            "a name somebody asks for is a preference, not an alias",
            any("锤子" in o for o in called),
            str(called),
        ),
        ...
        ("no episode copies the infected editorial style", not styled, str(styled)),
        ("the bot's own name never becomes a member's alias", not self_named, str(self_named)),
        ("nothing derives from the bot's own marked line", not own_derived, str(own_derived)),
    ]
    ...
    try:
        ...
        for candidate in cands:
            kind = {
                ...
                "GROUP_FACT": (
                    "record_group_term"
                    if candidate.payload.get("kind") == "term"
                    else "record_group_topic"
                ),
                "EPISODE": "record_episode",
            }[candidate.candidate_type.name]
            candidate.payload["_tool"] = kind

        print("candidates the model proposed:")
        for candidate in cands:
            print("  -", dict(candidate.payload))
        print()
