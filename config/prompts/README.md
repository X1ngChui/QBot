...
标记格式本身（`⟦图片:…⟧`、`#N`、`⟦同名N⟧` 等）、章节标题、机械性单行告知
...
旧方括号语法下「只能靠措辞恳求模型别上当」的局面；`output.clean_reply`
的末级兜底保证回复中不带保留括号出群。各标注的分工：

- 共用：`⟦时间⟧` 行首时间戳、`⟦同名N⟧` 同名成员固定编号、媒体与群事件标注。
- 仅回复路径：`#N` 行号、`⟦回复 #N⟧`、`⟦拥有者⟧`、`⟦依据:…⟧`、`⟦检索记录⟧`。
- 仅抽取路径：`⟦N⟧` 账号码（名册与发言行）、`⟦你⟧` 自有发言标记。

行边界锚定：转写行以 `#N ⟦时间⟧` 或 `⟦时间⟧` 开头，其余行是上一条的换行——
多行消息不能伪造行首。存量存档由 `scripts/migrate_markers.py` 一次性迁移；
旧式 `(N)` 同名后缀在旧存档文本里仍会出现，`identity_rules` 与
`extract_legend_note` 各留一句说明，`search_history` 的 speaker 参数两种
形态都接受。
...
- **`legend_reply_note`** — 回复侧补充：标注带描述即「已看到/听到」，须直接
  作答，不得让对方重复提问；附原图时以原图为准。回复窗口特有的标记（#N、
  ⟦回复 #N⟧、⟦依据:…⟧、⟦检索记录⟧、原图对应关系）住在这里——抽取的转写里
...
- **`extract_legend_note`** — 抽取侧补充：标注是系统转写痕迹。没有它，模型
  ...
  的标记级说明与 ⟦N⟧/⟦同名N⟧ 之辨也在这里。
- **`identity_rules`** — 账号/昵称/人三概念与昵称解析（含 ⟦同名N⟧ 与旧式
  (N) 的存档形态）。模型天然把字符串相似当作同一人的证据：会把重排昵称的
...
- **`inspect_image`** — 带问题重看原图的提问：只答图中可见、不猜测。
- **`extract`** — 记忆抽取的完整规则书。与工具 schema 强耦合（谓词名、
  ...
  复制回路；【本群固定资料】【拥有者注】是只读理解材料，quote 机械上不可能
...
- **`tool_read_url`** — 读网页正文；与搜索共用月额度，明说「先搜索后读页」
  的分工。
- **`tool_inspect_image`** — 唯一的付费工具：措辞把它压在「描述答不了、且问
  的是图中细节」之后，防止模型每见图必看、按张烧钱。
