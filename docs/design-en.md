...
4. **Group isolation**: every table carrying a group_id indexes it first, and no retrieval path crosses groups.
...
message arrives
 ...
 ↓ prompt assembly (cache-friendly ordering §6.2, text only; originals fetched on demand with open_images)
...
Five tools: `web_search` (Tavily, via proxy), `search_history` (boolean search over this group's L0 archive: Lucene syntax parsed by luqum - space=AND, OR, `-`exclusion, parentheses, quoted phrases, only the boolean subset accepted and the rest refused in words; the AST compiles into one parameterized ILIKE expression, member words entering SQL only as parameters; narrowable by speaker and days; each hit returns wrapped in `retrieval.history_context` lines of surrounding conversation each way - chat is fragments, and a matched line is routinely a bare answer to the line above it - with touching windows merged and blocks separated by an ellipsis line; the filters pick the hits, the context is whatever actually surrounds them; a matched message comes back whole, and the answer as a whole is bounded by `retrieval.history_chars` - hits x context lines x max_msg_len can outgrow the model's context, where a request fails outright instead of degrading, and a cut answer says so on its last line), `recall_events` (vector recall over this group's episodes; each recalled episode arrives framed by `retrieval.episode_context` neighbours either way in group time - an episode summarises one stretch of conversation, and its cause and consequence sit in the adjacent stretches - windows merged and blocks ellipsis-separated the same way, undated episodes standing alone), `read_url` (one page's readable text, off the same monthly search allowance), and `open_images` (originals by number, up to `retrieval.open_images_max` per call, handed to the model as file blocks). All five are free - a SQL query, an HTTP fetch, a file upload; the boundary is still money, because every round carrying them is a paid model call. Their division of labour is written into the tool descriptions and deliberately de-overlapped.
...
**Pictures are handled on arrival** (the CDN link is freshest then): download → an async one-line description backfills `plain_text`. The description is the archival form (design goal 8): search, extraction, restart-rebuild and the reply model until it opens a picture all consume text alone, and descriptions are written at arrival for every picture - including in groups the bot never answers in, where the reply path never runs. The reply model is multimodal, and nothing is pushed: every picture appears as a numbered description line (or a bare numbered marker), and the model fetches the pixels it wants with `open_images`, up to `retrieval.open_images_max` per call - one tool round on DeepSeek Flash is fast, and the history stays byte-stable so the cache no longer turns over when a new picture pushes an old one off a rail. The fetch rides on `image_cache.file_id` (uploaded free at arrival; an id older than `llm.vision.file_max_age_days` counts as expired and is uploaded again). Forwarded chat records - delivered inline in the segment by the protocol side, nested ones included - render as an indented block under the carrying message: a `⟦转发的聊天记录 N条⟧` header, one stamped and named line per entry, one indent level per nesting, bounded by `prompt.forward_lines/forward_depth/forward_chars` and closed with `⟦其余N条未显示⟧` past them; the pictures inside are the carrying message's own references, numbered and openable with the rest, filed for free and never described on their own account. The describing call is cached by image content (a repost costs nothing), rate-limited per minute, and behind the daily cap. The cached description expires after `llm.vision.description_ttl_days` (15): models improve, and a vendor can put a better one behind an unchanged id, so age is what tracks description quality - a model-id stamp would not see that. Refreshing is lazy by construction, because only a picture posted again is ever looked up: one nobody reposts is never paid for twice however old its description gets, and transcript lines already archived keep the wording they were written with. A picture the backend's content filter declined is cached as a marked placeholder on the same clock, so a later backend gets to look again - before this, that first refusal blinded the bot to that picture permanently. Pixels are fetched by number through the reply loop's `open_images` tool - every picture marker in the transcript carries its number (⟦图片3:description⟧), a position in one render like the line numbers. Picture references (`image_refs`) ride each message for the window's lifetime, are re-parsed from the archived payload after a restart, and QQ's file id trades for a fresh link at any time. The old `inspect_image`, which put the question to a second model and returned a sentence, is gone: once the model can look for itself the middleman is both redundant and worse, since it could only ever answer the one question it was given.
...
There is no token budget anywhere: every block renders whole, and each block's author (the owner, or a generation prompt with its own length discipline) is responsible for restraint. The only two numbers left are the window size and the eviction chunk, which serve finite context and the cache, not cost. No original rides in the prompt: pictures appear as numbered description lines and the model fetches what it wants to see with open_images (several per call), so every message renders byte-identical between turns and the cache never turns over on a picture.
...
Names are the model's only handle on people, and two members sharing a group card is ordinary. One rule (`core.namesakes`) decides who wears the reserved namesake tag (`名字⟦同名N⟧`) wherever names are rendered - the member-list refresh, the window relabel, the roster, search results: accounts whose bare names collide are tagged unless they all belong to one person (a merged main and alt share a name legitimately), and every account of a person wears that person's serial - the smallest of its accounts' permanent per-group serials (the `member_seq` table - assigned once, never reused, never reassigned, so a tagged name in any old transcript still points at the same person). Renames create new clashes and dissolve old ones, converging on the next refresh; the window relabel also tags a departed namesake's lines against the live member who still carries the name, so a bare name beside a tagged one never reads as the same account. `search_history`'s speaker argument accepts the tagged form and narrows to the serial's person - every account of theirs - rather than by name; its results tag namesakes the same way and mark the bot's own archived lines with `⟦你⟧`. The bot's own lines in the window and the archive open with `@asker`, as the group read them, so a later turn can see whom each answer was for. Extraction is untouched: its transcript already names accounts by per-batch code, and every write takes codes only.
...
- Observability: `/stats` (budget, call counts, monthly search allowance, per-use cache hit rates), `/groupstats`, the daily report (`schedule.report_cron`, midnight by default) (now carrying the output-strip counters - every hit is a near-miss leak, so source discipline regressing shows up in a report instead of an owner's eyeballs).
