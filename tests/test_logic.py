...
# -- the exit guard's pure half ----------------------------------------------
# The verdict mapping, with a fake judge; the end-to-end drop is pinned in
# test_pipeline. There is no fallback on purpose: an unreachable judge silences
# the reply rather than letting unjudged text out.
...
class _FakeVerdict:
    def __init__(self, s):
        self.suggestion, self.label, self.score = s, "Test", 88
...
class _FakeJudge:
    ...
    async def screen(self, text, *, group_id=None):
        if isinstance(self.outcome, Exception):
            raise self.outcome
        return _FakeVerdict(self.outcome)

    async def aclose(self):
        pass


def _screened(outcome):
    _censor.set_moderation(_FakeJudge(outcome))
    try:
        return _aio.run(_censor.screen("随便一句话"))
    finally:
        _censor.set_moderation(None)


check("no judge configured means no guard",
      _aio.run(_censor.screen("随便一句话")) == ("pass", ""))
check("a Block verdict holds the reply and says why",
      _screened("Block") == ("block", "Test/88"))
check("a Review verdict holds the reply", _screened("Review")[0] == "review")
check("a Pass verdict passes", _screened("Pass") == ("pass", ""))
check("a failed call holds the reply, fail-closed",
      _screened(RuntimeError("moderation down"))[0] == "error")

# The backend whitelist lives twice: providers/moderation.py holds the real
# table, and settings.ModerationCfg mirrors the names in its validator (it
# cannot import providers without a cycle). Pin the mirror in both directions
# it can drift: a real backend the validator rejects, and a typo it lets pass
# to detonate at the next restart.
from qqbot.providers import moderation as _pmod
from qqbot.settings import ModerationCfg as _MCfg

_accepted = []
for _name in list(_pmod.BACKENDS) + [""]:
    try:
        _MCfg(backend=_name)
        _accepted.append(_name)
    except Exception:
        pass
check("settings' backend whitelist covers the provider table",
      set(_accepted) == set(_pmod.BACKENDS) | {""}, str(_accepted))
_rejected = False
try:
    _MCfg(backend="no-such-backend")
except Exception:
    _rejected = True
check("a typo'd moderation backend is rejected at config load", _rejected)
